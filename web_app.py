...
try:
    ...
    print("✅ Aetherion V2 Backend extraction active (Observability, Health, Workspace, Execution, Admin)")
except ImportError as e:
    print(f"⚠️ Failed to load Aetherion V2 middlewares: {e}")
# -------------------------------------------------
...
_ALLOWED_SESSION_COLS = frozenset({
    "task", "model", "status", "success", "finished_at",
    "config", "mode", "log_tail", "score", "error",
    "started_at", "stage", "usage_json", "exit_code",
    "chain_id", "chain_task_id", "error_category", "files_json", "result",
})

def db_update_session(sid, **fields):
    if not fields: return
    safe = {k: v for k, v in fields.items() if k in _ALLOWED_SESSION_COLS}
    if not safe: return
...
def _p19_cors(response):
    """Apply CORS + security headers to every response."""
    response = apply_cors_headers(response, request)
    # Harden with standard security headers
...
_P8_COUPONS = {
    ...
    "NEXORA90":  {"plan": "pro",   "days": 90,  "note": "90-day Pro"},
    "NEXORA":    {"plan": "pro",   "days": 30,  "note": "30-day Pro"},
...
@app.route("/api/billing/webhook-status")
def api_billing_webhook_status():
    """Return Razorpay webhook configuration status and the webhook URL."""
    has_key_id     = bool(os.getenv("RAZORPAY_KEY_ID", "").strip())
    has_key_secret = bool(os.getenv("RAZORPAY_KEY_SECRET", "").strip())
    has_wh_secret  = bool(os.getenv("RAZORPAY_WEBHOOK_SECRET", "").strip())
    ...
    # Prefer REPLIT domain if available (production proxy)
    replit_domain = os.getenv("REPLIT_DEV_DOMAIN") or os.getenv("REPLIT_DOMAINS", "").split(",")[0].strip()
    if replit_domain:
        webhook_url = f"https://{replit_domain}/api/payments/webhook"
    else:
        webhook_url = f"{scheme}://{host}/api/payments/webhook"

    keys_ok    = has_key_id and has_key_secret
    fully_ok   = keys_ok and has_wh_secret
    ...
    if fully_ok:
        ...
        status = "partial"
        status_label = "⚠️ Keys set — webhook secret missing"
    else:
    ...
    return jsonify({
        ...
        "has_webhook_secret": has_wh_secret,
        "billing_available": _BILLING_AVAILABLE,
        "razorpay_enabled":  _pay.RAZORPAY_ENABLED if _BILLING_AVAILABLE else False,
    })
...
# --- PHASE Z31: PERSISTENT RUNTIME FORENSICS + EXECUTION MEMORY ---
try:
    from routes.forensics_z31 import forensics_z31_bp
    app.register_blueprint(forensics_z31_bp)
    print("✅ forensics_z31_bp registered (/api/z31/*)")
...
# --- PHASE Z38: PERSISTENT RUNTIME COGNITION + ADAPTIVE OPERATIONAL MEMORY ---
try:
    from routes.cognition_z38 import cognition_z38_bp
    app.register_blueprint(cognition_z38_bp)
    print("✅ cognition_z38_bp registered (/api/z38/*)")
except Exception as _z38_err:
    print(f"⚠ cognition_z38_bp skipped: {_z38_err}")

# --- PHASE Z39: RUNTIME STABILIZATION + COGNITIVE INTEGRITY ---
try:
    from routes.cognition_z39 import cognition_z39_bp
    app.register_blueprint(cognition_z39_bp)
    print("✅ cognition_z39_bp registered (/api/z39/*)")
...
# --- PHASE Z41: PREDICTIVE RUNTIME COORDINATION + LATENCY INTELLIGENCE ---
try:
    from routes.cognition_z41 import cognition_z41_bp
    app.register_blueprint(cognition_z41_bp)
    print("✅ cognition_z41_bp registered (/api/z41/*)")
